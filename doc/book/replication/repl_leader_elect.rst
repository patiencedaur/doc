...
This can be useful when an application
should not support master-master replication but it is necessary to
ensure that only one node accepts new transactions and commits them successfully.
...
Also, if an election is enabled on the node, it won't replicate from any nodes except
...
That must be especially actual when adding new nodes.
